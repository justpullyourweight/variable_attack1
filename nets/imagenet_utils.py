...
class ImageNetModel(ModelDesc):
    ...
    def inputs(self):
        return [tf.compat.v1.placeholder(self.image_dtype, [None, self.image_shape, self.image_shape, 3], 'input'),
                tf.compat.v1.placeholder(tf.int32, [None], 'label')]
    ...
    def build_graph(self, image, label):
        ...
        if self.weight_decay > 0:
            wd_loss = regularize_cost(self.weight_decay_pattern,
                                      tf.keras.regularizers.l2(0.5 * (self.weight_decay)),
    ...
    def compute_loss_and_error(logits, label, label_smoothing=0.):
        ...
        loss = tf.reduce_mean(input_tensor=loss, name='xentropy-loss')

        def prediction_incorrect(logits, label, topk=1, name='incorrect_vector'):
            with tf.compat.v1.name_scope('prediction_incorrect'):
                x = tf.logical_not(tf.nn.in_top_k(predictions=logits, targets=label, k=topk))
            return tf.cast(x, tf.float32, name=name)

        wrong = prediction_incorrect(logits, label, 1, name='wrong-top1')
        add_moving_summary(tf.reduce_mean(input_tensor=wrong, name='train-error-top1'))

        wrong = prediction_incorrect(logits, label, 5, name='wrong-top5')
        add_moving_summary(tf.reduce_mean(input_tensor=wrong, name='train-error-top5'))
        return loss
